...
function updateChartConfigurationUnified(
    SelectedchartType,
    selectedChartColor,
    selectedChartTransparency,
    selectedChartMarker,
    selectedChartGrid,
    selectedChartLegend,
    selectedFontSize
    ...
    // ==================================
    // SAVE ORIGINAL YAXIS ONCE
    // ==================================
    if (!window.originalYAxis) {
        window.originalYAxis = JSON.parse(
            JSON.stringify(chart.w.config.yaxis || [])
        );
    }
    ...
    const transparency =
        parseFloat(selectedChartTransparency) || 1;

    const markerSize =
        parseInt(selectedChartMarker) || 0;

    const showGrid =
        selectedChartGrid === 'true';

    const showLegend =
        selectedChartLegend === 'legendtrue';
    ...
    currentSeries.forEach((series, index) => {
        ...
        const isMovingAverage =
            seriesName.includes('movavg') ||
            seriesName.includes('moving') ||
            seriesName.includes('average');
        ...
        // ----------------------------
            ...
            const existingColor =
                chart.w.config.colors?.[index]
                ?? '#ff0000';

            updatedSeries.push({
                ...series,
                type: isMovingAverage
                    ? 'line'
                    : (series.type || 'column')
            });

            chartColors.push(existingColor);

            markerSizes.push(0);
            strokeWidths.push(2.25);
        }
    });

    // ==================================
    // SAFE COLORS
    // Prevent Apex toString() crash
    // ==================================
    const safeChartColors = chartColors.map(
        color => color || finalColor || '#2e75b6'
    );

    // ==================================
    // STROKE COLORS
    // Same behavior as ChartManager
    // ==================================
    const isSingleSeries =
        updatedSeries.length === 1;

    const useWhiteStroke =
        isSingleSeries &&
        (
    ...
    // MARKER COLORS
    // ==================================
    const markerColors = safeChartColors.map(
        (color, i) => {
    ...
    // ==================================
    // Y AXIS
    // Preserve original axis
    // ==================================
    const buildYAxis = (axis) => ({
        ...axis,
    ...
    const yAxisConfig =
        Array.isArray(window.originalYAxis)
            ? window.originalYAxis.map(buildYAxis)
            : buildYAxis(window.originalYAxis);
...
function applyChartSettings(type, clickedElement) {
  ...
  updateChartConfigurationUnified(settings.chartType, settings.chartType == 'line' ? '#ffffff' : '#' + settings.chartColor, settings.transparency != 1 ? '0.' + settings.transparency : settings.transparency, settings.marker.split("-")[1], settings.grid, settings.legend, settings.fontSize);
  }