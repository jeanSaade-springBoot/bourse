...
function activateChartTrasnparency(chartType) {
    if (chartType != 'area') disableChartColorTransparency(true);
    else disableChartColorTransparency(false);
}

function activateChartColor(chartType) {
    if (chartType == 'line') disableChartColor(true);
    else disableChartColor(false);
}

function activateChartMarker(chartType) {
    if (chartType != 'column') disableChartMarker(false);
    else disableChartMarker(true);
}
...
function updateGraphConfiguration(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend) { // console.log(SelectedchartType,selectedChartColor,selectedChartTransparency,selectedChartMarker,selectedChartGrid,selectedChartLegend)
...
function updateGraphConfigurationMissingConfiguration(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend) { // console.log(SelectedchartType,selectedChartColor,selectedChartTransparency,selectedChartMarker,selectedChartGrid,selectedChartLegend)
    activateChartTrasnparency(SelectedchartType);
    activateChartMarker(SelectedchartType);
    activateChartLegend(SelectedchartType);
    activateChartColor(SelectedchartType);
...
function updateGraphOption(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend) {
    ...
    if (typeof graphName != 'undefined' && (graphName == "wmqyVolume" || graphName == "RaceChartVolume")) updateGraphConfigurationVolumes(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend);
    else if (typeof hasMissingDates != 'undefined')
        if (hasMissingDates) updateGraphConfigurationMissingConfiguration(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend);
        else updateGraphConfiguration(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend);
    else updateGraphConfiguration(SelectedchartType, selectedChartColor, selectedChartTransparency, selectedChartMarker, selectedChartGrid, selectedChartLegend);
}
...
/*
 * ============================================================
 * GLOBAL 1-SCALE / 2-SCALES CONTROL
 * ============================================================
 *
 * The active segmented button is the initial source of truth. This keeps
 * the chart mode aligned with the HTML on first load even when an older
 * shared script initialized isOneScale to a different value.
 *
 * The legacy chartScaleOption() above is intentionally left unchanged for
 * existing .btn-option controls.
 */
function getGlobalScaleModeFromActiveButton() {
    const activeButton = document.querySelector("#scaleManagement .seg-btn.active");
    if (!activeButton) {
        return null;
    }
    const selectedScale = String(activeButton.getAttribute("data-value") || "").toLowerCase();
    if (selectedScale === "1scale") {
        return true;
    }
    if (selectedScale === "2scale") {
        return false;
    }
    return null;
}

function syncGlobalScaleStateFromActiveButton() {
    const activeMode = getGlobalScaleModeFromActiveButton();
    if (activeMode === null) {
        return;
    }
    isOneScale = activeMode;
}

function globalChartScaleOption(selectedChartScale) {
    const normalizedScale = String(selectedChartScale || "").toLowerCase();
    if (normalizedScale !== "1scale" && normalizedScale !== "2scale") {
        return;
    }
    /*
     * The clicked button becomes the UI source of truth first.
     */
    ...
    /*
     * Keep the legacy/shared variable synchronized for code paths that still
     * read isOneScale directly.
     */
    isOneScale = normalizedScale === "1scale";
    /*
     * For pages using the universal chartConfig implementation, changing the
     * scale must update ONLY the Y-axis. The existing rendered series contain
     * the artificial future dates that create the empty space on the right;
     * rebuilding the graph here would replace that padded data.
     */
    if (typeof applyUnifiedScaleOnly === "function") {
        const scaleResult = applyUnifiedScaleOnly();
        if (scaleResult && typeof scaleResult.catch === "function") {
            scaleResult.catch(function(error) {
                console.error("Error applying chart scale:", error);
            });
        }
    } else if (typeof refreshGraphNavigation === "function") {
        /* Compatibility fallback for pages that do not expose scale-only update. */
        const refreshResult = refreshGraphNavigation();
        if (refreshResult && typeof refreshResult.catch === "function") {
            refreshResult.catch(function(error) {
                console.error("Error refreshing chart scale:", error);
            });
        }
    } else if (typeof drawGraph === "function") {
        drawGraph();
    }
}
/*
 * Initialize the shared variable from the HTML selection before normal page
 * interaction. Pages that load chartConfig.js will also read the active
 * button directly through isUnifiedOneScaleMode().
 */
$(function() {
    syncGlobalScaleStateFromActiveButton();
});
...
function checkActiveChartColorTransparency(activeChartTransparency, chartTransparency) {
    if (typeof activeChartTransparency == 'undefined') {
        ...
        if (activeChartTransparency.id != 1) activeChartTransparencyID = '0.' + activeChartTransparency.id;
        else activeChartTransparencyID = activeChartTransparency.id;
...
function getChartColorTransparency(chartTransparency) {
    if (chartTransparency.split("0.").length == 1) nChartTransparency = chartTransparency.split("0.")[0];
    else nChartTransparency = chartTransparency.split("0.")[1];
...
function getTitlePeriodAndType() {
    if (getChartPeriodName() == 'DAILY') return '';
    else return ' - ' + getChartPeriodName() + ' ' + getSelectedTypeLabel();
}
    ...
    switch (period) {
        case 0:
            if (functionId === 4 || functionId === 9 || functionId === 11 || functionId === 13) {
                updateFunctionId = $("#dropDownFunctions").jqxDropDownList({
                    selectedIndex: 2
                });
