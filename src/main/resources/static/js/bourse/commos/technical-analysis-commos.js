...
var dropDownSource = [{ name: "GOLD", groupId: 6, subGroupId: 1,   ticker: "" },
	{ name: "SILVER", groupId: 6, subGroupId: 2,   ticker: "" },];
...
const chartStates = {
    ...
    chart4: createChartState() //chart4 in code 
};
...
$(document).ready(function() {
	...
	if (timeRange == "Daily") {
		...
		initializeFunctions(6, 1);
		
		//CryptosAnalisys
		// getTrendLinesHistory();
		initializeCryptoOptions(index,selectedLiveCurrency);
...
function initializeCryptoOptions(index, currency) {
	...
	$('#dropDownCryptoOptions').on('change', function(event) {
		...
		getTrendFollowingHistory(4,dropdownIds);
...
function buildCheckboxGroup(groupId, chartId) {
	...
	checkboxOptions[selectedLiveCurrency].forEach(opt => {
...
async function renderCheckboxesChart1VolumeFundingRate(mainGroupId, chartId = 2) {

		const container = document.getElementById(`checkboxes-container-chart-${chartId}`);
		container.innerHTML = '';

		const checkboxGroup = buildCandleStickCheckboxGroup(mainGroupId, chartId);
		...
		const allItems = filteredOptions.map(opt =>
			`#jqxCheckBox-${mainGroupId}-${opt.index}-chart-${chartId}`
		);
...
async function renderCheckboxesPerChart(mainGroupId, chartId = 2) {
	const cacheKey = `${mainGroupId}-${chartId}`;
	const container = document.getElementById(`checkboxes-container-chart-${chartId}`);
	...
	if (!checkboxCache[cacheKey]) {
		...
		const checkboxGroup = buildCheckboxGroup(mainGroupId, chartId);
		container.appendChild(checkboxGroup);

		const allItems = checkboxOptions[selectedLiveCurrency].map(opt =>
			`#jqxCheckBox-${mainGroupId}-${opt.index}-chart-${chartId}`
		);
        chartStates[`chart${chartId}`].allItems=allItems;
		...
		initializeItemsPerChart(allItems, chartId);
		let checkedIndex = selectedLiveCurrency=='GOLD'?1:2;
		// Pre-check logic
		if (chartId === 1) {
			allItems.forEach(id => {
				if (id.includes('-'+checkedIndex+'-')) {
		...
		if (chartId === 1) {
			...
			if (isCandleStick) {
				...
				let checkedIndex = selectedLiveCurrency === 'GOLD' ? 1 : 2;
					...
					checkboxOptions[selectedLiveCurrency].forEach(opt => {
					    const id = `#jqxCheckBox-${mainGroupId}-${opt.index}-chart-${chartId}`;
					
					    if (opt.index === checkedIndex) {
				 ...
				 if (timeRange !== '4h')
				 $(`#jqxCheckBox-${mainGroupId}-funding_rate-chart-${chartId}`).hide();
			} else {
...
function getDataChart1() {
	...
	const manager = ChartManager.instances[chartKey] || new ChartManager(chartKey, options, `#commos${chartId}-container`);
	...
	manager.render().then(() => {
		  ...
		  `);
		    ...
		    const selectedGroupsId = mainGroupId;
           ...
           if(isChecked && timeRange==='Daily')
           {
              ...
              saveTrendLineHistory(isShared,1,BuySelldropdownIds); //isShared
			   ...
			   if ( chartStates.chart1.trendFollowingLoading) return;   // ✅ stop re-entry
				...
				params[`subGroupId1`] = selectedLiveCurrency=='GOLD'?1:2;
				params[`groupId1`] = mainGroupId;
			...
			const api = timeRange === "Daily"
				? "/graph/get-weighted-trend-graph"
...
function techAnalysisCheck(){
	...
	if(isChecked)
 			{  $("#dropDownFunctions").jqxDropDownList('clearSelection');
				...
				$('#groupOfPeriod-chart1').removeClass("d-flex").addClass("invisible");
				
				let checkedIndex = selectedLiveCurrency === 'GOLD' ? 1 : 2;
	
				checkboxOptions[selectedLiveCurrency].forEach(opt => {
				    const id = `#jqxCheckBox-${mainGroupId}-${opt.index}-chart-1`;
 			...
 			{  
 			  ...
 			  //  $('#candlestickToggle-chart1').removeClass("d-none").addClass("d-block"); 
 			    ...
 			    $('#groupOfPeriod-chart1').removeClass("invisible").addClass("d-flex"); 

 			    if(isCandleStick)
 			    {
					 renderCheckboxesPerChart(mainGroupId, 1);
				 }
 			     else{
					let checkedIndex = selectedLiveCurrency === 'GOLD' ? 1 : 2;
					...
					$(`#jqxCheckBox-${mainGroupId}-funding_rate-chart-1`).hide();
					}
 			}
...
function getDataChart4() { // trendfollowing
	...
	const chartId = '4';
	const manager = new ChartManager(`chart${chartId}`, options, `#commos${chartId}-container`);
	...
	manager.render().then(() => {
		...
		dropdownIds.forEach(id => {
		  ...
		  $(`#${id}`).off('select').on('select', function () {
		    ...
		    if (chartStates.chart4.isProgrammaticDropdownUpdate || chartStates.chart4.isRefreshingDropdowns || chartStates.chart4.isBulkUpdatingDropdowns)
		      return;
		    ...
		    chartStates.chart4.suppressTrendFollowingReload = true;
		    // Reload chart only for user-initiated event
		    ...
		    // Refresh UNIQUE selection logic
		    chartStates.chart4.isRefreshingDropdowns = true;
		    setTimeout(() => {
		      refreshAllDropdowns(dropdownIds);
		      chartStates.chart4.isRefreshingDropdowns = false;
		    });
		...
		$('input[name="options"]').on('change', function () {
		  ...
		  if (group) {
		    chartStates.chart4.isBulkUpdatingDropdowns = true;
		    ...
		    chartStates.chart4.isBulkUpdatingDropdowns = false;
...
async function updateTrendFollowingGraph(chartId, manager, saveHistory) {
	if ( chartStates.chart4.trendFollowingLoading) return;   // ✅ stop re-entry
   		chartStates.chart4.trendFollowingLoading = true;
  ...
  if(saveHistory)
     saveTrendLineHistory(isShared,4,dropdownIds); //isShared
    ...
    const commonParams = {
        ...
        type: '3',
        groupId1: selectedGroupsId,
        subGroupId1: selectedLiveCurrency=='GOLD'?1:2,
        removeEmpty1: false,
    ...
    const api = "/graph/get-weighted-trend-graph";
	let titleA = dropDownSource.find(c => c.groupId === commonParams[`groupId1`] && c.subGroupId === subGroupId).name;
    ...
    if (candlestickIsActive) {
        ...
        await manager.loadData({
            service: manager._lastService || "graph",
            api: api,
        ...
        await manager.loadData({
		   ...
		   chartStates.chart4.trendFollowingLoading = false;
        setTimeout(() => chartStates.chart4.suppressTrendFollowingReload = false, 200);
    }
...
function toggleGraphData(time) {
	...
	const selectedGroupId = mainGroupId;
...
function initializeFunctions(groupId, subgroupId = null){
     
   let api = subgroupId!=null ? `/admin/getfunctions/${groupId}?subgroupId=${subgroupId}` : `/admin/getfunctions/${groupId}` ;
...
function getTrendFollowingHistory(chartId, dropdownIdsArrays){
	...
	$.ajax({
		...
		url: "/graph/find-trend-following-history-by-userid-groupId-and-subgroupId-and-chartId/"+selectedGroupsId+`/${subGroupId}/${isShared}/${chartId}`,
		dataType: 'json',
		async: true,
		cache: false,
		timeout: 600000,
...
function loadHistoryAndFillDropdowns(data) {
   ...
   chartStates.chart4.isProgrammaticDropdownUpdate = true;
    ...
    // 4) Radio buttons logic stays based on full list
    setTimeout(() => {
        chartStates.chart4.isProgrammaticDropdownUpdate = false;
        refreshAllDropdowns(dropdownIds);
...
async function saveTrendLineHistory(isShared, chartId, dropdownIdsArrays) {
        ...
        let entity = {
            ...
            isShared: isShared ,
            groupId:  mainGroupId,
            subGroupId: selectedLiveCurrency=='GOLD'?1:2,
			chartId: chartId
        };
