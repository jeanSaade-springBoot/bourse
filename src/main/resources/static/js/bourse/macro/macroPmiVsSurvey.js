...
$(document).ready(function() {
	  ...
	  $.ajax({
		...
		for (let i = 0; i < groupId1.length; i++) {
					...
					country= getCountryImagePath(groupId1[i].groupId.toString());
				 	groupContainer+="<div class='col-2 d-grid left'>"+
				 		"<img src='"+country[0]+"' width='40px' height='40px' class='align-middle'>"+"<div class='align-middle font-weight-bold fs-7'>"+country[1]+"</div>";
				...
				if(groupId1[i].subgroupId==1)
					checkBox1="jqxCheckBox-"+groupId1[i].groupId+'-'+'2';
				else if(groupId1[i].subgroupId==3) 
					checkBox1="jqxCheckBox-"+groupId1[i].groupId+'-'+'4';
			...
			groupContainer+="</div>";	
	        	...
	        	for (j = 0; j < groupId1.length; j++) {
					items="#jqxCheckBox-"+groupId1[j].groupId+'-'+groupId1[j].subgroupId;
		    	  	allitemsLeft.push(items);
		    	  	$(items).jqxCheckBox({ theme: 'dark', width: '100%', height: 26 });
		    	  	if(groupId1[j].subgroupId==1)
		    	  		items1="#jqxCheckBox-"+groupId1[j].groupId+'-'+'2';
		    	  		else 
		    	  		  	if(groupId1[j].subgroupId==3)
		    	  			items1="#jqxCheckBox-"+groupId1[j].groupId+'-'+'4';
		    	  			
		    	  	allitemsLeft.push(items1);
	...
	$.ajax({
	        contentType: "application/json",
	        url: "/macro/get-macro-display-final-with-flash",
	        dataType: 'json',
	        async:true,
	        cache: false,
	        timeout: 600000,
		...
		for (let i = 0; i < groupId1.length; i++) {
					...
					country= getCountryImagePath(groupId1[i].groupId.toString());
				 	groupContainer+="<div class='col-2 d-grid right'>"+
				 		"<img src='"+country[0]+"' width='40px' height='40px' class='align-middle'>"+"<div class='align-middle font-weight-bold fs-7'>"+country[1]+"</div>";
				
				isVisibleClass=(data[i].isVisible)?"d-block":"d-none";
				
				checkBox="flash-jqxCheckBox-"+groupId1[i].groupId+'-'+groupId1[i].subgroupId;
				groupContainer+='<div id="'+checkBox+'" class="'+isVisibleClass+' jqx-checkbox-items align-middle"></div>';
				
				if(groupId1[i].subgroupId==1)
					checkBox1="flash-jqxCheckBox-"+groupId1[i].groupId+'-'+'2';
				else if(groupId1[i].subgroupId==3) 
					checkBox1="flash-jqxCheckBox-"+groupId1[i].groupId+'-'+'4';
					
				groupContainer+='<div id="'+checkBox1+'" class="'+isVisibleClass+' jqx-checkbox-items align-middle"></div>';
			
			groupContainer+="</div>";	
		} 
   			  $('#countriesflash').append(groupContainer);
			
	        	for (j = 0; j < groupId1.length; j++) {
					items="#flash-jqxCheckBox-"+groupId1[j].groupId+'-'+groupId1[j].subgroupId;
		    	  	allitemsRight.push(items);
		    	  	$(items).jqxCheckBox({ theme: 'dark', width: '100%', height: 26 });
		    	  	if(groupId1[j].subgroupId==1)
		    	  		items1="#flash-jqxCheckBox-"+groupId1[j].groupId+'-'+'2';
		    	  		else 
		    	  		  	if(groupId1[j].subgroupId==3)
		    	  			items1="#flash-jqxCheckBox-"+groupId1[j].groupId+'-'+'4';
		    	  			
		    	  	allitemsRight.push(items1);
		    	  	$(items1).jqxCheckBox({ theme: 'dark', width: '100%', height: 26 });
				}
...
function drawGraph() {
 ...
 function macroGraph(graphService,graphName,removeEmpty,saveHistory){
	...
	chart.render();
	 ...
	 {    const subgroupId=itemValue[checkedItemValues[0]].subGroupId;
	      const groupId=itemValue[checkedItemValues[0]].GroupId;
		   ...
		   dataParam = { 
		 		        	    ...
		 		        	    "groupId1": groupId,
		 		        	    "subGroupId2":subgroupId,
		 		        	    "groupId2": groupId,
		 		        	    "subGroupId3":subgroupId,
		 		        	    "groupId3": groupId,
		 		        	    "factor1": 16,
		 		        	    "factor2": 15,
		 		        	    "factor3": 14,
