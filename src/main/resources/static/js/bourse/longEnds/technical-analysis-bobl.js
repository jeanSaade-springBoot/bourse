...
var dropDownSource = [{ name: "INITIALS", groupId: "53", ticker: "INITIALS" },
	{ name: "ROLLING", groupId: "62", ticker: "ROLLING" },];
...
async function renderCheckboxesPerChart(cryptoGroupId, chartId = 2, renderBoth = true) {
  ...
  const groupsToRender = renderBoth
    ? dropDownSource.filter(g => g.groupId === '53' || g.groupId === '62')
  ...
  if (!checkboxCache[cacheKey]) {
    ...
    allItems.forEach(id => {
      if (id.includes('62-4-')) $(id).jqxCheckBox('check');
    });
