...
let selectedLiveCurrency = 'BOBLS'; // default
let mainLabel = "BOBLS ( OHLC - 21:00 CLOSE )";
...
var chartHeight=635;
const screenName='BOBLSAnalisys';
const graphName='BOBLSAnalisys';
...
var dropDownSource = [{ name: "INITIALS", groupId: "54", ticker: "INITIALS" },
	{ name: "ROLLING", groupId: "63", ticker: "ROLLING" },];
...
async function renderCheckboxesPerChart(cryptoGroupId, chartId = 2, renderBoth = true) {
  ...
  const groupsToRender = renderBoth
    ? dropDownSource.filter(g => g.groupId === '54' || g.groupId === '63')
  ...
  if (!checkboxCache[cacheKey]) {
    ...
    allItems.forEach(id => {
      if (id.includes('63-4-')) $(id).jqxCheckBox('check');
    });
...
if (trendFollowingLoading) return;   // ✅ stop re-entry
  ...
  if(saveHistory)
	...
	let titleA = 'BOBLS ( OHLC - 21:00 CLOSE )';//dropDownSource.find(c => c.groupId === commonParams[`groupId1`]).name;
