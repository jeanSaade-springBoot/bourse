...
$(document).ready(function() {
     ...
     $("#show").click(function() { 
		...
		if(checkedItemLeft>0 || checkedItemRight>0)
		{
	     ...
	     getGraphHistoryByScreenName("usJobsGraph");
...
function drawGraph() {
 	      ...
 	      const removeEmpty = false;
  	      usJobsGraph(graphService,"usJobsGraph",removeEmpty,true);	
}
	...
	for (let i = 0; i < checkedItemid.length; i++) {
				if (checkedItemid[i] != null)
					{checkedItemValues.push(checkedItemid[i]);
					 ...
					 if(value[2]=='1')
					 {
					   checkedItemValues.push(value[0]+'-'+value[1]+'-'+'2');
					 }
					}
			}
