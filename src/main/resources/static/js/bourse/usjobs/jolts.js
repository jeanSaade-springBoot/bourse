var allitems = ["#jqxCheckBoxfinal-77", "#jqxCheckBoxinitial-77", "#jqxCheckBoxsurv-77", ];
...
function renderFunction(groupId) {
    ...
    switch (selectedChart) {
        ...
        default:
            // Original behaviour
            if (isChecked) currentUsJobsFunction(groupId);
            else getGraphUsJobData(graphService, graphName, removeEmpty, true);
            break;
    }
}
...
function multiplySeriesBy1000(responseIndex, response) {
    ...
    response[responseIndex].config.displayDescription = response[responseIndex].config.displayDescription + " (Thousands)";
}
...
function joltsJobOpeningsVsUnemployment(groupId) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            min1 = Math.min.apply(null, response[0].graphResponseDTOLst.map(item => item.y).filter(y => y != null && !isNaN(y)));
            max1 = Math.max.apply(null, response[0].graphResponseDTOLst.map(item => item.y).filter(y => y != null && !isNaN(y)));
            min2 = Math.min.apply(null, response[1].graphResponseDTOLst.map(item => item.y).filter(y => y != null && !isNaN(y)));
            max2 = Math.max.apply(null, response[1].graphResponseDTOLst.map(item => item.y).filter(y => y != null && !isNaN(y)));
            ...
            let yaxisArray = [{
                ...
                labels: {
                    ...
                    formatter: function(val, index) {
                        if (yAxisFormat1[1]) return val.toFixed(yAxisFormat1[0]);
                        else return val.toFixed(yAxisFormat1[0]) + "%";
                    }
                },
                tickAmount: 6,
            ...
            let tooltipArray = {
                ...
                y: {
                    formatter: function(value, {
                        ...
                        value = Number(value);
                        if (seriesIndex == 0) {
                            return getFormatResult0[1] ? value.toFixed(getFormatResult0[0]) : value.toFixed(getFormatResult0[0]) + "%";
                        }
                        if (seriesIndex == 1) {
                            return getFormatResult1[1] ? value.toFixed(getFormatResult1[0]) : value.toFixed(getFormatResult1[0]) + "%";
                        }
...
function joltsOpeningsMinusUnemployedFunction(groupId) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#overlayChart').hide();
            $("#mainChart-title").empty();
            $("#mainChart-title").html('<div id="title-image" style="position:absolute;top: 5px;left:25%;height:60px;background: #172568;" class="title-style">' + '<img height="50" class="pr-2" src="' + getCountryImagePath("77")[0] + '">' + '<span style="color:white;font-weight:bold;">US Monthly </span>' + '<span style="color:#ffd400;font-weight:bold;">JOB OPENINGS(JOLT) minus UNEMPLOYED LABOR FORCE </span>' + '</div>');
        },
        error: function(e) {
            console.log("ERROR : ", e);
        }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            chart.updateOptions({
                ...
                tooltip: tooltipArray,
                annotations: {
                    yaxis: annotationArray
                },
            });
            disableChartFont(false);
            $('#overlayChart').hide();
            $("#mainChart-title").empty();
            $("#mainChart-title").html('<div id="title-image" style="position:absolute;top: 5px;left:25%;height:60px;background: #172568;" class="title-style">' + '<img height="50" class="pr-2" src="' + getCountryImagePath("77")[0] + '">' + '<span style="color:white;font-weight:bold;">US Monthly </span>' + '<span style="color:#ffd400;font-weight:bold;">JOB OPENINGS to UNEMPLOYED LABOR FORCE </span>' + '<span style="color:white;font-weight:bold;">RATIO</span>' + '</div>');
        },
        error: function(e) {
            console.log("ERROR : ", e);
        }
