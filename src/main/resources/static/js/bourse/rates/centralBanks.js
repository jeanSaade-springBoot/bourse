var allitems = ["#jqxCheckBoxfed_lower_rate", "#jqxCheckBoxfed_upper_rate", "#jqxCheckBoxfed_lower_move", "#jqxCheckBoxfed_upper_move", "#jqxCheckBoxecb_depo_rate", "#jqxCheckBoxecb_refi_rate", "#jqxCheckBoxecb_lending_rate", "#jqxCheckBoxecb_depo_move", "#jqxCheckBoxecb_refi_move", "#jqxCheckBoxecb_lending_move", "#jqxCheckBoxboe_refi_move", "#jqxCheckBoxboe_monthly_base_rate"];
...
$(document).ready(function() {
    // Central Banks must start in 1-SCALE mode so the initial chart
    // matches the default UI state. This runs only once when the page loads;
    // subsequent 1-SCALE / 2-SCALE button clicks can still change isOneScale.
    if (typeof isOneScale === 'undefined') {
        window.isOneScale = true;
    } else {
        isOneScale = true;
    }
    // Keep the scale toggle UI synchronized with the initial chart state.
    $("#scaleManagement .seg-btn").removeClass("active").attr("aria-pressed", "false");
    $("#scaleManagement .seg-btn[data-value='1scale']").addClass("active").attr("aria-pressed", "true");
    ...
    if (checkedItem == 4) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const isMove1 = checkedItemValues[0].toLowerCase().includes("move");
                const isMove2 = checkedItemValues[1].toLowerCase().includes("move");
                ...
                calculatedMinValue = (min >= 0 && calculatedMinValue < 0) ? 0 : calculatedMinValue;
                var yaxisformat0 = getFormat(response[0].config.yAxisFormat);
                var yaxisformat1 = getFormat(response[1].config.yAxisFormat);
                // Respect the shared 1-SCALE / 2-SCALES state from chartConfig.js.
                const useOneScale = (typeof isUnifiedOneScaleMode === "function") ? isUnifiedOneScaleMode() : ((typeof isOneScale === "undefined") ? true : !!isOneScale);

                function calculateCentralBanksAxisBounds(rawMin, rawMax) {
                    const margin = addMarginToMinMax(rawMin, rawMax, 5);
                    let axisMin = Math.sign(rawMin) == -1 ? -Math.abs(rawMin) - margin : Math.abs(rawMin) - margin;
                    const axisMax = Math.sign(rawMax) == -1 ? -Math.abs(rawMax) + margin : Math.abs(rawMax) + margin;
                    // If the actual series has no negative values, never let the
                    // calculated margin push the displayed minimum below zero.
                ...
                const combinedAxisBounds = calculateCentralBanksAxisBounds(min, max);
                const firstAxisBounds = calculateCentralBanksAxisBounds(min1, max1);
                const secondAxisBounds = calculateCentralBanksAxisBounds(min2, max2);
                const centralBanksYAxis = useOneScale ? {
                    labels: {
                        minWidth: 75,
                        maxWidth: 75,
                        style: {
                        ...
                        formatter: function(val) {
                            return yaxisformat0[1] ? val.toFixed(yaxisformat0[0]) : val.toFixed(yaxisformat0[0]) + "%";
                        }
                    },
                    tickAmount: 6,
                    min: combinedAxisBounds.min,
                    max: combinedAxisBounds.max,
                    axisBorder: {
                        width: 3,
                        show: true,
                        color: '#ffffff',
                    ...
                    seriesName: response[0].config != null ? (response[0].config.displayDescription == null ? '' : response[0].config.displayDescription) : '',
                    opposite: false,
                    labels: {
                        minWidth: 75,
                        maxWidth: 75,
                    ...
                    tickAmount: 6,
                    min: firstAxisBounds.min,
                    max: firstAxisBounds.max,
                    axisBorder: {
                        width: 3,
                        show: true,
                        color: '#ffffff',
                    ...
                    seriesName: response[1].config != null ? (response[1].config.displayDescription == null ? '' : response[1].config.displayDescription) : '',
                    opposite: true,
                    labels: {
                        minWidth: 75,
                        maxWidth: 75,
                        ...
                        formatter: function(val) {
                            return yaxisformat1[1] ? val.toFixed(yaxisformat1[0]) : val.toFixed(yaxisformat1[0]) + "%";
                        }
                    },
                    tickAmount: 6,
                    min: secondAxisBounds.min,
                    max: secondAxisBounds.max,
                    axisBorder: {
                        width: 3,
                        show: true,
                        color: '#ffffff',
                ...
                const rateColors = ['#F0AB2E', '#F0AB2E'];
                ...
                const data0 = checkedItemValues[0].toLowerCase().includes("move") ? response[0].graphResponseDTOLst : carryForwardRateData(response[0].graphResponseDTOLst);
                const data1 = checkedItemValues[1].toLowerCase().includes("move") ? response[1].graphResponseDTOLst : carryForwardRateData(response[1].graphResponseDTOLst);
                chart.updateOptions({
                    series: [{
                    ...
                    colors: colorsArray,
                    fill: {
                        opacity: checkedItemValues.map(item => item.toLowerCase().includes("move") ? 1 : 0.5)
                    },
                    stroke: {
                        colors: checkedItemValues.map((item, index) => item.toLowerCase().includes("move") ? colorsArray[index] : "#ffffff"),
                        width: StrokeSizes,
                    },
                    markers: {
                        colors: colorsArray,
                            ...
                            formatter: function(value, {
                                ...
                                if (seriesIndex == 1) {
                                    if (getFormatResult1[1]) return value.toFixed(getFormatResult1[0]);
                                        ...
                                        if (value != null) return value.toFixed(getFormatResult1[0]) + "%";
                                    }
                                }
                            },
                            title: {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const isMoveSeries = checkedItemValues[0].toLowerCase().includes("move");
                chartType1 = isMoveSeries ? 'line' : 'area';
                chartColor = isMoveSeries ? "#ffffff" : "#F0AB2E";
                fontsize = checkActiveFontSize($("#fontOptions").find(".active")[0], chartDbFontSize);
                markerSize = checkActiveChartMarker($("#chartMarker").find(".active")[0], response[0].config.chartshowMarkes);
                showGrid = checkActiveChartGrid($("#gridOptions").find(".active")[0], response[0].config.chartShowgrid);
                showLegend = checkActiveChartLegend($("#gridLegend").find(".active")[0], showLegend);
                ...
                chart.updateOptions({
                    ...
                    fill: {
                        opacity: checkedItemValues[0].toLowerCase().includes("move") ? 1 : 0.5
                    },
                    stroke: {
...
function renderMoreThanTwoSeries(response) {
    ...
    // Gold area for rates; move marker colors remain unchanged.
    arrayColors = checkedItemValues.map((item, index) => item.toLowerCase().includes("move") ? (arrayColors[index] || "#ff0000") : "#F0AB2E");
    const dates = [];
    const series = [];
    const minValues = [];
    const maxValues = [];
    ...
    response.forEach((item, index) => {
        ...
        const isMove = checkedItemValues[index].toLowerCase().includes("move");
        let chartType = isMove ? 'line' : 'area';
        series.push({
    ...
    chart.updateOptions({
        ...
        stroke: {
            colors: checkedItemValues.map((item, index) => item.toLowerCase().includes("move") ? arrayColors[index] : "#ffffff"),
            width: checkedItemValues.map(item => item.toLowerCase().includes("move") ? 0 : 2),
        },
        markers: {
            colors: arrayColors,
            strokeColors: arrayColors,
            size: checkedItemValues.map(item => item.toLowerCase().includes("move") ? 2 : 0),
        },
        yaxis: {
            labels: {
                minWidth: 75,
        ...
        tooltip: {
            ...
            y: {
                formatter: function(value, {
                    ...
                    if (seriesIndex == 0) {
                        if (formatResult[1]) return value.toFixed(formatResult[0]);
                        else return value.toFixed(formatResult[0]) + "%";
                    } else {
                        if (formatResult[1]) return value.toFixed(formatResult[0]);
                        else {
                            if (value != null) return value.toFixed(formatResult[0]) + "%";
                        }
                    }
                },
                title: {
